...
async function getTotalRegistrations() {
    try {
        const { count, error } = await tonSupabase
...
app.get('/api/ton/register', async (req, res) => {
    try {
        ...
        console.log(`🔄 TON Bot: Registering user: ${userId}, ${userUsername}`);

        // Check if user already exists in our tracking table
        const { data: existingUser, error: checkError } = await tonSupabase
            .from('ton_bot_registrations')
            .select('*')
            .eq('user_id', userId)
            .single();
        ...
        console.log(`✅ TON Bot: Registration result:`, result);
        res.json(result);

    } catch (error) {
        console.error('❌ TON Bot: Server error:', error);
        res.status(500).json({
            success: false,
            message: 'Internal server error',
            error: error.message
...
app.get('/api/ton/stats', async (req, res) => {
    try {
        ...
        console.error('TON Bot: Error getting stats:', error);
        res.status(500).json({
            success: false,
...
app.get('/api/ton/registrations', async (req, res) => {
    try {
        ...
        console.error('TON Bot: Error getting registrations:', error);
        res.status(500).json({
            success: false,
...
// Health check endpoint for TON API
app.get('/api/ton/health', (req, res) => {
    res.json({
        success: true,
        message: 'TON Bot API is running',
        timestamp: new Date().toISOString()
    });
});
// ==================== END TON BOT REGISTRATION API ====================
